<?php
$query = "SELECT MAX_COUNTER FROM trainowner WHERE OWNER_ID='$user_id'";
$result = mysqli_query($con, $query);
$row = mysqli_fetch_array($result);
$maxcounter = $row['MAX_COUNTER'];
$query = "SELECT count(COUNTER_ID) AS REG_COUNTER FROM traincounter WHERE OWNER='$user_id'";
$result = mysqli_query($con, $query);
$row = mysqli_fetch_array($result);
...
if (isset($_POST['register'])) {
    ...
    $acquery = "INSERT INTO traincounter (COUNTER_ID,OWNER,ADDRESS,CONTACT) VALUES ('$id','$owner','$address','$phone')";

    if (mysqli_query($con, $loginquery) && mysqli_query($con, $acquery)) {
        echo "<script>alert('Successfully Registered COUNTER ID( $id ).');location.href = 'addtraincounter.php';</script>";
    } else {
