<?php
$query = "SELECT MAX_COUNTER FROM airowner WHERE OWNER_ID='$user_id'";
$result = mysqli_query($con, $query);
$row = mysqli_fetch_array($result);
$maxcounter = $row['MAX_COUNTER'];
$query = "SELECT count(COUNTER_ID) AS REG_COUNTER FROM aircounter WHERE OWNER='$user_id'";
$result = mysqli_query($con, $query);
$row = mysqli_fetch_array($result);
...
<form target="_self" enctype="multipart/form-data" method="POST">
    <div class="col">
        <div class="form-group">
            <label for="usr">AIRPORT NAME</label>
...
if (isset($_POST['register'])) {
    ...
    $acquery = "INSERT INTO aircounter (COUNTER_ID,OWNER,ADDRESS,CONTACT) VALUES ('$id','$owner','$address','$phone')";

    if (mysqli_query($con, $loginquery) && mysqli_query($con, $acquery)) {
        echo "<script>alert('Successfully Registered COUNTER ID( $id ).');location.href = 'addaircounter.php';</script>";
    } else {
